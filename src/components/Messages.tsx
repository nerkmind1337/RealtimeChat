...
const Messages: FC<MessagesProps> = ({
    ...
    useEffect(() => {
        ...
        return () => {
            pusherClient.unsubscribe(toPusherKey(`chat:${chatId}:incoming-message`))
            pusherClient.unbind('iincoming-message', messageHandler);
        }
    }, [])
    ...
    return <div id='messages' className='flex h-full flex-1 flex-col-reverse gap-4 p-3 overflow-y-auto scrollbar-thumb-blue scrollbar-thumb-rounded scrollbar-track-blue-lighter scrollbar-w-2 scrolling-touch'>
        ...
        {messages.map((message, index) => {
            ...
            const hasNextMesageFromSameUser = messages[index - 1]?.senderId === messages[index].senderId;
                    ...
                    <div className={cn('flex items-end', {
                        ...
                        <div className={cn('flex flex-col space-y-2 text-base max-w-xs mx-2', {
                            ...
                            <span className={cn('px-4 py-2 rounded-lg inline-block', {
                                ...
                                'rounded-br-none': !hasNextMesageFromSameUser && isCurrentUser,
                                'rounded-bl-none': !hasNextMesageFromSameUser && !isCurrentUser,
                            })}>
                                {message.text}{' '}
                        ...
                        <div className={cn('relative w-6 h-6 ', {
                            ...
                            'invisible': hasNextMesageFromSameUser
                        })}>
                            <Image
                                fill
